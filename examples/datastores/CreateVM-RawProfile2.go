...
func main() {
	...
	// new code
	dummyVMName := "MARVEL-checkyou-prashanth121"
	...
	// task, err := virtualDiskManager.MoveVirtualDisk(ctx)
	...
	// 				ThinProvisioned: types.NewBool(true),
	// 				VirtualDeviceFileBackingInfo: types.VirtualDeviceFileBackingInfo{
	// 					FileName: "[" + datastorePathObj.Datastore + "]",
	// 				},
	// 			},
	// 		},
	// 	},
	...
	// vm1, err := f.VirtualMachine(ctx, "master")

	// host1, err := vm1.HostSystem(ctx)
	// resourcepool, err := host1.ResourcePool(ctx)
	// tw := tabwriter.NewWriter(os.Stdout, 2, 0, 2, ' ', 0)
	// fmt.Fprintf(tw, "%s\t", resourcepool)
	...
	vm, err := f.VirtualMachine(ctx, dummyVMName)
	// if err != nil {
	// 	exit(err)
	// }
