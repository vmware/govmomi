...
func main() {
	...
	// new code
	dummyVMName := "MARVEL-checkyou-prashanth159"
	vmVirtualMachineConfigSpec := types.VirtualMachineConfigSpec{
		Name: dummyVMName,
		Files: &types.VirtualMachineFileInfo{
			VmPathName: "[" + datastorePathObj.Datastore + "] kubevols",
		},
		NumCPUs:  1,
		MemoryMB: 2048,
	}
	...
	// 	vmFolderRefs = vmFolderRefs(refs, vmFolder.Reference())
	// }
	...
	// 		if fref.Name == "k8s-mine" {
	// 			myFolder = object.NewFolder(c.Client, folderRef)
	// 		}
	// 	}
	// }
	...
	pc := property.DefaultCollector(c.Client)
	err = pc.Retrieve(ctx, vmFolderRefs, []string{"name"}, &folderMorefs)
	if err != nil {
		fmt.Println("hello")
		exit(err)
	}
	for index, fref := range folderMorefs {
		if vmFolderRefs[index].Type == "folder" && fref.Name == "k8s-mine" {
			resultFolder = object.NewFolder(c.Client, vmFolderRefs[index])
		}
	}

	task, err := resultFolder.CreateVM(ctx, vmVirtualMachineConfigSpec, resourcepool, nil)
	if err != nil {
