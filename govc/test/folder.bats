...
@test "folder.place" {
    ...
    assert_failure # -placementType must be specified

    run govc folder.place $name -vm $vmid -pool $poolid -placementType relocate
    assert_success

    run govc folder.place -json $name -vm $vmid -pool $poolid -placementType relocate
    assert_success
}
