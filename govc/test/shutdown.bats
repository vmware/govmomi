...
@test "shutdown.poweroff" {
  ...
  status=$(govc vcsa.shutdown.status| grep -i "Action"| awk '{print $2}')
...
@test "shutdown.status" {
  ...
  run govc vcsa.shutdown.status
  assert_success

  run govc vcsa.shutdown.status -json
  assert_success
...
@test "shutdown.cancel" {
  ...
  status=$(govc vcsa.shutdown.status| grep -i "Action"| awk '{print $2}')
  assert_equal $status ''
  status=$(govc vcsa.shutdown.status| grep -i "ShutDownTime"| awk '{print $2}')
  assert_equal $status ''
  status=$(govc vcsa.shutdown.status| grep -i "Reason"| awk '{print $2}')
