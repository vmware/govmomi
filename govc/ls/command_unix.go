...
// Function To escape spaces, parentheses for easier unix use (think copy/paste)
func EscapePath(path string) string {

    // replace whitespace with an escaped space
	r := regexp.MustCompile(`(\s)+`)
	path = r.ReplaceAllString(path, `\ `)

    // replace an open parentheses with an escaped parentheses
	r = regexp.MustCompile(`(\()`)
	path = r.ReplaceAllString(path, `\(`)

    // replace a closed parentheses with an escaped closed parentheses
	r = regexp.MustCompile(`(\))`)
	path = r.ReplaceAllString(path, `\)`)
...
func (l listResult) Write(w io.Writer) error {
	...
	for _, e := range l.Elements {
		...
		case mo.Folder:
			if _, err = fmt.Fprintf(w, "%s/\n", EscapePath(e.Path)); err != nil {
				return err
			}
		default:
			if _, err = fmt.Fprintf(w, "%s (%s)\n", EscapePath(e.Path), e.Object.Reference().Type); err != nil {
				return err
			}
		}
	}
