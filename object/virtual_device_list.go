...
// SATAControllerTypes are used for adding a new SATA controller to a VM.
func SATAControllerTypes() VirtualDeviceList {
	// Return a mutable list of SATA controller types, initialized with defaults.
	return VirtualDeviceList([]types.BaseVirtualDevice{
		&types.VirtualSATAController{},
	}).Select(func(device types.BaseVirtualDevice) bool {
		c := device.(*types.VirtualSATAController).GetVirtualSATAController()
		c.BusNumber = -1
		return true
	})
}
...
func (l VirtualDeviceList) FindSATAController(name string) (types.BaseVirtualController, error) {
	if name != "" {
		...
		case *types.VirtualAHCIController:
			return &c.VirtualSATAController, nil
		default:
...
func (l VirtualDeviceList) CreateSATAController() (types.BaseVirtualDevice, error) {
	sata := &types.VirtualSATAController{}
